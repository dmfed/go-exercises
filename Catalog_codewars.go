...
func Catalog(s string, article string) string {
	item := regexp.MustCompile(`<name>(.+)</name><prx>(.+)</prx><qty>(.+)</qty>`)
	searchres := item.FindAllStringSubmatch(s, -1)
	result := []string{}
	for _, val := range searchres {
		if strings.Contains(val[1], article) {
			n := fmt.Sprintf("%v > prx: $%v qty: %v", val[1], val[2], val[3])
...
func main() {
	fmt.Println(Catalog(testData, "saw"))
}

var testData = `<prod><name>drill</name><prx>99</prx><qty>5</qty></prod>
