...
from s2. In the following we will not consider letters when the maximum
...
func Mix(s1, s2 string) string {
	...
	filterLowercase(s1, letters)
	filterLowercase(s2, letters)
	out := []string{}
	for letter := range letters {
		count1, count2 := strings.Count(s1, letter), strings.Count(s2, letter)
		if count1 == count2 && count1 > 1 {
			out = append(out, "=:"+strings.Repeat(letter, count1))
		} else if count1 > count2 && count1 > 1 {
			out = append(out, "1:"+strings.Repeat(letter, count1))
		} else if count2 > count1 && count2 > 1 {
			out = append(out, "2:"+strings.Repeat(letter, count2))
		}
	}
...
func filterLowercase(s string, dict map[string]bool) {
	for _, letter := range s {
		if unicode.IsLower(letter) {
			dict[string(letter)] = true
		}
	}
}

func main() {
	fmt.Println(Mix("looping is fun but dangerous", "less dangerous than coding") == "1:ooo/1:uuu/2:sss/=:nnn/1:ii/2:aa/2:dd/2:ee/=:gg")
	fmt.Println(Mix("my&friend&Paul has heavy hats! &", "my friend John has many many friends &") == "2:nnnnn/1:aaaa/1:hhh/2:mmm/2:yyy/2:dd/2:ff/2:ii/2:rr/=:ee/=:ss")
}
